...
{
    /// <summary>
    /// 依存関係編集全体を統括するViewModel。
    /// UI状態、ノード・接続線管理、Undo/Redo、データ入出力などを扱う。
    /// </summary>
    public class DependencyEditorViewModel : ViewModelBase
    {
        ...
        /// </summary>
        public TaskNodeCollectionViewModel Nodes { get; }
        ...
        /// <summary>
        /// ノードとそのポート一覧を関連付ける辞書。
        /// PortViewModel は TaskNodeViewModel に依存しないため、
        /// 所属ノードの逆引きに使用する。
        /// </summary>
        public Dictionary<TaskNodeViewModel, List<PortViewModel>> NodePorts { get; } = [];
        ...
        private TaskNodeViewModel.TaskStatus CalculateTaskStatus(TaskNodeViewModel node)
        {
            var now = DateTime.Now;

            if (node.Detail.EndDateTime != null && node.Detail.EndDateTime <= now)
            {
                return TaskNodeViewModel.TaskStatus.Done;
            }

            if (IsInSuspensionPeriod(node, now) || GetUpstreamNodes(node).Any(upstream => !IsDone(upstream, now)))
            {
                return TaskNodeViewModel.TaskStatus.Pending;
            }

            if (node.Detail.StartDateTime != null && node.Detail.StartDateTime <= now)
            {
                return TaskNodeViewModel.TaskStatus.InProgress;
            }

            return TaskNodeViewModel.TaskStatus.Ready;
        }
        ...
        private static bool IsDone(TaskNodeViewModel node, DateTime now)
        {
        ...
        private static bool IsInSuspensionPeriod(TaskNodeViewModel node, DateTime now)
        {
        ...
        private IEnumerable<TaskNodeViewModel> GetUpstreamNodes(TaskNodeViewModel node)
        {
        ...
        private TaskNodeViewModel? FindNodeByPort(PortViewModel port)
        {
        ...
        private void RegisterNodeForStatusRefresh(TaskNodeViewModel node)
        {
        ...
        private void UnregisterNodeForStatusRefresh(TaskNodeViewModel node)
        {
            node.Detail.PropertyChanged -= OnNodeDetailStatusPropertyChanged;
            node.Detail.SuspensionPeriods.CollectionChanged -= OnSuspensionPeriodsChanged;
        ...
        {
            if (e.PropertyName is nameof(TaskDetailViewModel.StartDateTime) or nameof(TaskDetailViewModel.EndDateTime))
            {
                RefreshTaskStatuses();
            }
        }
        ...
        public void RequestNodeSizeChanged(object? dataContext, Size newSize)
        {
            ...
            node.Width = Math.Max(TaskNodeViewModel.MinWidth, newSize.Width);
            node.Height = Math.Max(TaskNodeViewModel.MinHeight, newSize.Height);
        ...
        public void LoadFromTaskEditorDataModel(TaskEditorDataModel data)
        {
            ...
            NodePorts.Clear();
            DependencyEditorViewModel loadedData = DependencyEditorMapper.ToViewModel(data, this);
        ...
        {
            data = DependencyEditorMapper.ToDataModel(this);
        }
        ...
        /// <summary>
        /// DependencyEditorViewModel を生成し、各管理クラスを初期化する。
        /// </summary>
        public DependencyEditorViewModel()
        {
            Nodes = new TaskNodeCollectionViewModel(UndoRedo, DateTimeEditor, this);
