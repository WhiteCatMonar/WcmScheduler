...
複数世代のバックアップを保持する場合、古い世代は `project.backup.1.json`、`project.backup.2.json` のように世代番号を付与します。
...
アプリ設定ファイルでは、`auto-backup-generation-count` に保持世代数を保存します。
バックアップ世代数は、メニューバーの `ツール` > `設定` から変更します。
